import {Component, ChangeDetectionStrategy, Input, Output, EventEmitter, HostBinding, OnChanges} from '@angular/core';
...
export class TagsSelectComponent implements OnChanges {
  ...
  get hasFilteredTags() {
    return this.filterTags && this.filterTags.length > 0 ? 'block' : 'none';
  }
  ...
  filterTags() {
    ...
    this.filteredTags = this.tags
      .filter(tag => tag.hashTag.toLowerCase().includes(filter) ||
